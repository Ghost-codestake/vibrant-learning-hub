...
type AssignmentDetail = { course: string; due: string; points: string; graded: number; total: number; overdue: number; brief: string; recent: Array<{ initials: string; name: string; state: string; tone: "rose" | "amber" | "teal" | "muted" }> };
const assignmentDetails: Record<string, AssignmentDetail> = {
  "ML Project 3": { course: "Intro to Machine Learning", due: "Overdue · 5 Sept", points: "100 points", graded: 26, total: 48, overdue: 6, brief: "Train and evaluate a classifier on the provided dataset, then submit a short model report.", recent: [{ initials: "MN", name: "Maria N.", state: "Overdue", tone: "rose" }, { initials: "TA", name: "Tunde A.", state: "Ungraded", tone: "amber" }, { initials: "SR", name: "Sofia R.", state: "Graded · 96", tone: "teal" }] },
  "Data Structures · Assignment 7": { course: "Advanced Data Structures", due: "Due today · 7 Sept", points: "60 points", graded: 18, total: 32, overdue: 2, brief: "Implement balanced tree insertion and deletion, with complexity analysis for each operation.", recent: [{ initials: "JT", name: "James T.", state: "Ungraded", tone: "amber" }, { initials: "DO", name: "Daniel O.", state: "Graded · 94", tone: "teal" }, { initials: "CW", name: "Chen W.", state: "Ungraded", tone: "amber" }] },
  "Cloud Architecture · Lab 4": { course: "Cloud Architecture 101", due: "Due Friday · 11 Sept", points: "40 points", graded: 9, total: 27, overdue: 0, brief: "Deploy a highly available multi-region service and document the failover strategy.", recent: [{ initials: "AP", name: "Aisha P.", state: "Ungraded", tone: "amber" }, { initials: "EM", name: "Elena M.", state: "Graded · 88", tone: "teal" }, { initials: "LK", name: "Liam K.", state: "Not submitted", tone: "muted" }] },
  "API Design · Quiz 5": { course: "API Design Patterns", due: "Scheduled · 15 Sept", points: "25 points", graded: 0, total: 44, overdue: 0, brief: "Timed quiz covering versioning, pagination, idempotency, and error contract design.", recent: [{ initials: "LK", name: "Liam K.", state: "Not started", tone: "muted" }, { initials: "SR", name: "Sofia R.", state: "Not started", tone: "muted" }, { initials: "MN", name: "Maria N.", state: "Not started", tone: "muted" }] },
};

function AssignmentDrawer({ title, onClose }: { title: string; onClose: () => void }) {
  const detail = assignmentDetails[title];
  const [action, setAction] = useState<string | null>(null);
  if (!detail) return null;
  const pct = Math.round((detail.graded / detail.total) * 100);
  const toneClass = { rose: "bg-rose/15 text-rose", amber: "bg-amber/15 text-amber", teal: "bg-teal/15 text-teal", muted: "bg-muted text-muted-foreground" };
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <button aria-label="Close details" className="absolute inset-0 bg-ink/80 backdrop-blur-sm" onClick={onClose} />
      <aside role="dialog" aria-label={`${title} details`} className="animate-rise relative flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-border bg-panel p-5">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-medium uppercase text-brand">{detail.course}</p><h2 className="mt-1 text-xl font-semibold">{title}</h2><p className="mt-1 text-xs text-muted-foreground">{detail.due} · {detail.points}</p></div>
          <Button variant="dashboard" size="icon" aria-label="Close" onClick={onClose}>×</Button>
        </div>
        <p className="mt-4 text-sm leading-6 text-muted-foreground">{detail.brief}</p>
        <div className="mt-5 grid grid-cols-3 gap-3">
          <div className={cn(panel, "p-3")}><p className="text-lg font-semibold">{detail.total}</p><p className="text-[11px] text-muted-foreground">Submissions</p></div>
          <div className={cn(panel, "p-3")}><p className="text-lg font-semibold text-teal">{detail.graded}</p><p className="text-[11px] text-muted-foreground">Graded</p></div>
          <div className={cn(panel, "p-3")}><p className={cn("text-lg font-semibold", detail.overdue ? "text-rose" : "text-muted-foreground")}>{detail.overdue}</p><p className="text-[11px] text-muted-foreground">Overdue</p></div>
        </div>
        <div className="mt-5">
          <div className="flex justify-between text-[13px]"><span className="font-medium">Grading progress</span><span className="text-muted-foreground">{pct}%</span></div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="animate-progress h-full rounded-full bg-brand" style={{ width: `${pct}%` }} /></div>
        </div>
        <h3 className="mt-6 text-[15px] font-semibold">Recent submissions</h3>
        <div className="mt-2 space-y-1">{detail.recent.map((student) => <div key={student.name} className="flex items-center gap-3 rounded-[10px] px-2 py-2 hover:bg-muted"><Avatar initials={student.initials} /><span className="flex-1 text-[13px] font-medium">{student.name}</span><span className={cn("rounded-lg px-2 py-1 text-[10px] font-semibold", toneClass[student.tone])}>{student.state}</span></div>)}</div>
        <h3 className="mt-6 text-[15px] font-semibold">Quick actions</h3>
        <div className="mt-3 grid grid-cols-2 gap-2">
          {["Start grading", "Message class", "Extend deadline", "Download submissions"].map((label, index) => <Button key={label} variant={index === 0 ? "brand" : "dashboard"} onClick={() => setAction(label)}>{label}</Button>)}
        </div>
        {action && <p className="mt-3 rounded-xl bg-teal/10 px-3 py-2 text-xs text-teal ring-1 ring-teal/20">Demo: “{action}” triggered for {title}.</p>}
      </aside>
    </div>
  );
}
...
export function CollectionPage({ kind }: { kind: CollectionKind }) {
  const data = pageData[kind]; const [query,setQuery] = useState(""); const [created,setCreated] = useState(false); const [selected,setSelected] = useState<string | null>(null);
  const rows = data.rows.filter((row) => row.join(" ").toLowerCase().includes(query.toLowerCase()));
  return <InstructorShell><main className="mx-auto max-w-[1400px] px-4 py-6 md:px-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-medium uppercase text-brand">{data.eyebrow}</p><h1 className="mt-1 text-3xl font-semibold">{data.title}</h1><p className="mt-2 text-sm text-muted-foreground">{data.description}</p></div><Button variant="brand" onClick={() => setCreated(true)}><Plus />{kind === "courses" ? "New course" : kind === "assignments" ? "New assignment" : "Add item"}</Button></div>{created && <div className="mt-5 flex items-center justify-between rounded-xl bg-teal/10 px-4 py-3 text-sm text-teal ring-1 ring-teal/20"><span>Demo item added to this view.</span><button aria-label="Dismiss" onClick={() => setCreated(false)}>×</button></div>}<div className={cn(panel,"mt-6 overflow-hidden")}><div className="flex flex-wrap items-center gap-3 border-b border-border p-4"><div className="relative min-w-52 flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder={`Search ${data.title.toLowerCase()}…`} className="h-9 w-full rounded-[10px] bg-muted pl-9 pr-3 text-sm outline-none ring-1 ring-border focus:ring-brand/50"/></div><Button variant="dashboard">Filter</Button><Button variant="dashboard">Export</Button></div><div className="divide-y divide-border">{rows.map((row,index)=><button key={row[0]} onClick={kind === "assignments" ? () => setSelected(row[0]) : undefined} className="grid w-full grid-cols-[1fr_auto] items-center gap-4 p-4 text-left transition hover:bg-muted md:grid-cols-[1.6fr_1fr_1fr_1fr_auto]"><div className="flex items-center gap-3"><Avatar initials={String(index+1).padStart(2,"0")}/><span className="text-sm font-semibold">{row[0]}</span></div>{row.slice(1).map((cell)=><span key={cell} className="hidden text-sm text-muted-foreground md:block">{cell}</span>)}<ChevronRight className="size-4 text-muted-foreground"/></button>)}{rows.length === 0 && <div className="p-10 text-center text-sm text-muted-foreground">No matching items found.</div>}</div></div></main>{selected && <AssignmentDrawer title={selected} onClose={() => setSelected(null)} />}</InstructorShell>;
}
